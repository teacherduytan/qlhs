...
| C133 | ✅ Xong | 24/07/2026 | Rà lại giao diện Điểm danh/Sĩ số trên điện thoại: modal sửa điểm danh tự cuộn, thu hẹp cột lưới tuần, sửa khối Nội trú/Bán trú/Hai buổi bị chèn chữ; sửa lỗi Safari iOS chặn tab Google Form vì window.open() gọi sau await |
| C134 | ✅ Xong | 24/07/2026 | Sửa mất lịch sử liên lạc phụ huynh khi sửa lại điểm danh: `lien_lac_phu_huynh.diem_danh_id` đổi từ `on delete cascade` sang `on delete set null`, thêm cột chụp nhanh `ma_hs`/`ho_ten`/`ngay`/`buoi` ngay lúc ghi để lịch sử không mất khi dòng `diem_danh` gốc bị xoá lúc chuyển về Có mặt |
